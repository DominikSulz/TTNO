...
%     for jj=1:(2^(l-1))
%         tmp = jj*(2^(l-1) + 2) + 1;
%         mat_C(tmp,count) = 1;    % A_j (from the left subtree)
%         count = count + 1;
%     end
%     for jj=2:(2^(l-1))
%          mat_C(jj,count) = 1;    % A_j (from the right subtree)
%          count = count + 1;
%     end
%     
%     mat_C(2^(l-1)+2,2^l+2) = 1;     % A_i \otimes A_j which are only at the right subtree
...
%     for ii=1:(2^(l-1))
%         for jj=1:(2^(l-1))
%             mat_C(ii*(2^(l-1)+2) + jj + 1,2^l+2) = V(pos(ii),pos(2^(l-1)+jj));  % A_i \otimes A_j, A_i from left and A_j from right subtree
%         end
%     end
%    
%     TTNO{3} = eye(2^l+2,2^l+2);
%     TTNO{4} = mat2tens(mat_C.',[2^(l-1)+2 2^(l-1)+2 2^l+2],1:2,3);
% else
%     % root tensor
%     len = length(A);
%     s = len/2;
...
%     for ii=1:(2^(l-1))
%         for jj=1:(2^(l-1))
%             mat_C(ii*(2^(l-1)+2) + jj +1) = V(pos(ii),pos(2^(l-1)+jj));  % A_i \otimes A_j, A_i from left and A_j from right subtree
%         end
%     end
%     
%     mat_C(2^(l-1)+2) = 1;                           % \sum Ai*Aj from right subtree
...
% %     mat_C = reshape(mat_C,[2^(l-1)+2 2^(l-1)+2]); 
%     P = mat_C;
% %     mat_C = mat2tens(mat_C.',[2^(l-1)+2 2^(l-1)+2 1],1:2,3); % mat2tens(mat_C.',[2^(l-1)+2 2^(l-1)+2 1],3,1:2)
%     
%     TTNO{end} = tensor(mat_C,[2^(l-1)+2 2^(l-1)+2 1]);
%     
% end
